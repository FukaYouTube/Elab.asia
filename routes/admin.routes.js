...
app.action(/./gm, async (ctx, next) => {
    ...
    paginate(clients, 3 , parseInt(ctx.callbackQuery.data), clients.length / 3).arr.map(client => {
        message += StringParser.rules(messages['clients-info']['body-text'], { client }) + '\n'
    })

    ctx.reply(message, inlineKeyboard(paginate(clients, 3, parseInt(ctx.callbackQuery.data), clients.length / 3).keys.map(b => callbackButton(b.text, b.callback))).extra())
})
